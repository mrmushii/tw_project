...
- **Skills live at `workspace/skills/<name>/` in *both* environments** — `~/.openclaw/workspace/skills/` locally, `/app/workspace/skills/` in the container. The Dockerfile used to copy them to `/app/skills`, so the workspace-relative path in `SKILL.md` resolved to a real file locally and a missing one on the deployed instance — the skill reported "skill files not found" on Render/Railway only, while testing clean locally. Keep the layouts identical. Also **never invoke a skill script by a cwd-relative path**: the agent's cwd is not the workspace on the scheduled path. The commands in `SKILL.md` resolve an absolute path via an `[ -f ]` test on both candidates.
- **`SKILL.md` must never tell the agent to curl the GitHub API.** `$GITHUB_TOKEN` is blank in the exec environment (see the stripping gotcha below), so the curl 401s and Haiku concludes — and reports — that the token is unconfigured, which is false. Every credentialed call goes through `check.py`, which reads the dotenv itself. `check.py commits -n <N>` answers "what was the last commit?" read-only, without consuming activity as seen.
